...
@section('content')
			...
			<div id="page-wrapper">
				...
				<div class="container-fluid">
					...
					<form enctype="multipart/form-data" method="POST" action="{{ route('admin.faq.update',$faqs->id) }}" novalidate>
					@csrf
									...
									<div class="row">
										<div class="col-sm-3">
											...
											<select class="form-control @error('type') is-invalid @enderror" name="type" required>
												...
												<option value="Member" {{ $faqs->type == 'Member' ? 'selected' : '' }}>Member</option>
												<option value="Company" {{ $faqs->type == 'Company' ? 'selected' : '' }}>Company</option>
											</select>
											@error('type')
										...
										<div class="col-sm-4">
											<label>Judul</label>
											<input type="text" class="form-control @error('title') is-invalid @enderror" name="title" value="{{old('title',$faqs->title)}}" required>
											@error('title')
												<div class="invalid-feedback">{{ $message }}</div>
											@enderror										
										</div>										
							...
							<div class="card">
								...
								<div class="card-body">
									<div class="row">
										...
										<div class="col-sm-12">
										{{--<label></label>--}}
											<textarea class="form-control @error('content') is-invalid @enderror" id="summernote" placeholder="Isi Konten Lengkap Halaman" name="content" required>{!!old('content',$faqs->content)!!}</textarea>
											@error('content')
												<div class="invalid-feedback">{{ $message }}</div>
											@enderror											
										</div>
									...
									<div class="row">
											...
											<div class="input-group">
												<div class="profile-image-outer-container">
													<div class="profile-image-inner-container bg-color-primary">
														@if(empty($faqs->picture))
															<img id="profile-img-tag" src="{{asset('front/img/default/cover.jpg') }}" class="img-responsive img-circle">
														@else
															<img id="profile-img-tag" src="{{asset('storage/uploads/faq/'.$faqs->picture) }}" class="img-responsive">										
														@endif	
													</div>
										...
										<div class="col-sm-2">
											...
											<select class="form-control @error('active') is-invalid @enderror" name="active" required>
												...
												<option value="1" {{ $faqs->active == '1' ? 'selected' : '' }}>Aktif</option>
												<option value="0" {{ $faqs->active == '0' ? 'selected' : '' }}>Non Aktif</option>
											</select>
											@error('active')
												<div class="invalid-feedback">{{ $message }}</div>
											@enderror											
