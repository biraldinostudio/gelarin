...
@section('title', trans('mail.Please check the incoming application email', ['appName' => config('app.name')]))
@section('content')
...
<p style="font-family: 'Helvetica Neue', 'Helvetica', Helvetica, Arial, sans-serif; font-size: 14px; line-height: 1.6em; font-weight: normal; margin: 0 0 10px; padding: 0;">{!! $data['bodyMessage'] !!}<br><br>@lang('mail.post_employer_contacted_content_1', [
	    'name'      => $data['name'],
	    'email'      => $data['userCreatorEmail'],
	    'phone'      => $data['telp'],
	    'title'      => ucwords($data['subject']),		
		'postUrl' => url('vacancies/detail',[$data['id'],$data['slug']])
	])
