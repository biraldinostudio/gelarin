							...
							<div class="blog-sidebar">
								...
								<div class="sidebar-widget">
									...
									<ul class="sidebar-list">
										@foreach($categories as $category)
											<li>
												<a href="{{route('article.byCategory',[$category->translation_of,$category->slug])}}">{{$category->name}}<span></span></a>
											</li>
										@endforeach
									</ul>
								</div>
								...
								<div class="sidebar-widget">
									...
									@foreach($artPopulars as $article)
										@if($article->visits>5)									
									<div class="blog-item">
										<div class="post-thumb"><a href="{{route('article.detail',[$article->id,$article->slug])}}"><img src="{{asset('storage/uploads/article/'.$article->cover)}}" class="img-responsive" alt=""></a></div>
										<div class="blog-detail">
											<a href="{{route('article.detail',[$article->id,$article->slug])}}"><h4>{{ str_limit(strip_tags(ucwords($article->title)), 50) }}</h4></a>
