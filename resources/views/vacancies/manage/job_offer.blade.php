					...
					<div class="row">
						...
						<div class="col-lg-8 col-md-8">
							...
							<div class="row-bottom">
								<h2 class="detail-title">@lang('vacancy.Job Offer')</h2>
								<form role="form" enctype="multipart/form-data" method="GET" action="{{ route('manage.vacancies.offer') }}" role="search">
								@csrf
									...
									<div class="mng-resume">
										<div class="col-md-2 col-sm-2">
											...
											<a href="#" target="_blank">
												<img src="{{ asset('storage/uploads/company/logo/'.$jobOffer->logo) }}" class="img-responsive" alt="#" /></a>
											</div>
										</div>
										<div class="col-md-8 col-sm-8">
											<div class="mng-resume-name">
												<h4><a href="{{route('vacancies.detail',[$jobOffer->id,$jobOffer->slug])}}" target="_blank">{{ str_limit(strip_tags(ucwords($jobOffer->title)), 50) }}</a> <span class="cand-designation">({{$jobOffer->visits}} @lang('vacancy.Visits'))</span></h4>
												<span class="cand-status">{{ str_limit(strip_tags(ucwords($jobOffer->company)), 21) }}</span> (<span style="color:#11b719;">{{date($jobOffer->date_format, strtotime($jobOffer->created_at))}}</span>)<br>
												<span>{{$jobOffer->types}}</span>,
												<span>{{ str_limit(strip_tags(ucwords($jobOffer->city)), 21) }}<span>						
											</div>
										</div>
										<div class="col-md-2 col-sm-2">
											<div class="mng-resume-action">
												<a href="{{route('application.apply',[$jobOffer->id,$jobOffer->slug])}}" class="btn advance-search" title="apply">@lang('vacancy.Apply')</a>
											</div>
										</div>
									</div>
								</article>								
